...
mod tests {
    ...
    #[test]
    fn test_is_memory_block_detects_injected_prefix() {
        let rendered = render_memory(&[fact(1, "use cargo test", 0, None, false)], "", 2048);
        // Same shape the runtime injects (`memory_block_for`).
        let block = format!("{MEMORY_BLOCK_START}\n{rendered}{}\n{MEMORY_BLOCK_END}", "");
        assert!(is_memory_block(&block));
        // Plain user text is never a memory block.
        assert!(!is_memory_block("fix the login bug"));
        assert!(!is_memory_block(""));
        // Memory marker inside a larger message is NOT a prefix block.
        let mid = format!("intro\n{block}\nend");
        assert!(!is_memory_block(&mid));
    }
